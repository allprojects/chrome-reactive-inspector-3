\section{Evaluation of Improvements}
	...
	% - All fields are only displayed on a node if they have a value. 
	% - The node Ids are continuous and start with 1.
	% - No console errors from extension scripts while using the test application. (the selenium webdriver may have console errors, due to the reload of the page by CRI)
	...
	%#### specification how the tool should be have (kind of generic) #behavior
	%- asses that basic behaviors happen in all applications instead of test for specifics
	%- what are the 10-15 most important behavior we expect from the tool	
	
	%- spreadsheet with details about every application, name, source, short description, length, technologies.
	%- notify guido if ready
